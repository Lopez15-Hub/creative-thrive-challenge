...
class ShowPopupBloc extends Bloc<ShowPopupEvent, bool> {
  ShowPopupBloc() : super(false) {
    on<ShowPopupEvent>((event, emit) {
     ...
     if(state)  customPopupWidget(event.context, 'Are you sure?', 'You must create the item later',event.categoryId, productId: event.productId, categories: event.categories);
